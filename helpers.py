...
# CREATE KEYPOINTS
def extract_keypoints(entradaManos):
    if entradaManos.left_hand_landmarks:
        manoIzq = []
        for i in entradaManos.left_hand_landmarks.landmark:
            manoIzq.extend([i.x, i.y, i.z])
        manoIzq = np.array(manoIzq)
    else: 
        manoIzq = np.zeros(63)
        
    if entradaManos.right_hand_landmarks:
        manoDere = []
        for i in entradaManos.right_hand_landmarks.landmark:
            manoDere.extend([i.x, i.y, i.z])
        manoDere = np.array(manoDere)
    else: 
        manoDere = np.zeros(63)
    return np.concatenate([manoIzq, manoDere])
